...
TOTAL_VIDEO_CHUNCK = 159
BITRATE_LEVELS = [400,800,1000,1500,2500,4000]
VIDEO_PATH = 'bbb_30fps_'
# VIDEO_FOLDER = 'video'
...
for bitrate,folder in enumerate(BITRATE_LEVELS):
	...
	with open('video_size_' + str(len(BITRATE_LEVELS)-bitrate-1), 'w') as f:
		for chunk_num in tqdm(range(1, TOTAL_VIDEO_CHUNCK+1)):
			...
			# 	)[0]
			video_chunk_path = glob.glob(VIDEO_PATH+str(folder)+'k/*_'+str(chunk_num)+'.m4v')[0]
